...
class Engine:
    ...
    def parse_workers(self, workers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        for worker in workers:
            ...
            worker['Frei-Tage'] = worker['Frei-Tage'].split(', ') if worker['Frei-Tage'] else []
            worker['No-Go-Schichten'] = self.parse_no_go_slots(worker['No-Go-Schichten'])
            worker['Fix-Tage'] = worker['Fix-Tage'].split(', ') if pd.notna(worker['Fix-Tage']) else []
            worker['shift_count'] = 0  # Initialize shift count
        return workers
    ...
    def fill_slots(self):
        for date, details in self.workdays_and_slots.items():
            ...
            for slot in available_slots:
                assigned_worker = self.assign_worker_to_slot(date, slot, assigned_workers, weekday)
                if assigned_worker:
                    assigned_workers.append(assigned_worker)
                else:
                    print(f"Warning: Could not fill slot on {date}.")

    def assign_worker_to_slot(self, date: str, slot: Dict[str, Any], 
                               assigned_workers: List[Dict[str, Any]], weekday: str) -> Optional[Dict[str, Any]]:
    ...
    def can_assign_worker(self, worker: Dict[str, Any], date: str, slot: Dict[str, Any], 
                          assigned_workers: List[Dict[str, Any]], weekday: str) -> bool:
        ...
        if worker['Folgetag'] == "Ja" and self.is_consecutive_day(assigned_workers, date):
            return True

        return False

    def is_consecutive_day(self, assigned_workers: List[Dict[str, Any]], date: str) -> bool:
        pass
